...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    *)
      ...
      echo "    --bottleneck_dim: 瓶颈维度 (默认: 256)"
...
# 创建输出目录
OUTPUT_DIR="output/mcc/${MODEL_NAME}/${DATA_NAME}"
LOGS_DIR="logs/mcc/${MODEL_NAME}/${DATA_NAME}"
...
echo "开始运行 $DATA_NAME 数据集上的 MCC 实验: $SOURCE -> $TARGET \(第 $RUN 次实验, 种子: $SEED\), 模型: $MODEL, GPU: $GPU"
...
# 运行MCC实验
CUDA_VISIBLE_DEVICES=$GPU python mcc.py $DATADIR \
--data_name $DATA_NAME --data_folder_name $DATA_NAME \
--source $SOURCE --target $TARGET --model_type "$MODEL" --i $ITERS_PER_EPOCH \
--epochs $EPOCHS --seed $SEED --log_file "$LOG_FILE" --bottleneck-dim $BOTTLENECK_DIM 2>&1 | tee $TEMP_OUTPUT
...
echo "$DATA_NAME 数据集上的 MCC 实验 $SOURCE -> $TARGET \(第 $RUN 次实验, 种子: $SEED\) 已完成。" 