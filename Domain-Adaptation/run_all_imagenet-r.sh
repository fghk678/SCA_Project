...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    *)
      echo "未知参数: $1"
      echo "用法: $0 [--gpus gpu_ids] [--concurrent 并行数] [--runs 实验次数]"
...
# ImageNet-R的源域和目标域
SOURCE_DOMAINS=("IN" "IN-val")
TARGET_DOMAIN="INR"

# 创建输出目录
OUTPUT_DIR="output/proposed/clip/imagenet-r"
mkdir -p $OUTPUT_DIR
...
for SOURCE in "${SOURCE_DOMAINS[@]}"; do
    # 检查JSON文件是否存在
    JSON_FILE="${OUTPUT_DIR}/${SOURCE}_${TARGET_DOMAIN}.json"
    if [ -f "$JSON_FILE" ]; then
        # 读取现有结果
        EXISTING_SEEDS=$(python -c "
import json
import sys
try:
...
")
    ...
    echo "已有的 $SOURCE -> $TARGET_DOMAIN 实验种子: $EXISTING_SEEDS"
    ...
    for RUN in $(seq 1 $NUM_RUNS); do
        ...
        if [[ "$EXISTING_SEEDS" == *"$SEED"* ]]; then
            echo "跳过已完成的实验: $SOURCE -> $TARGET_DOMAIN (种子: $SEED)"
        else
            TASKS+=("$SOURCE,$TARGET_DOMAIN,$SEED,$RUN")
            echo "添加任务: $SOURCE -> $TARGET_DOMAIN (种子: $SEED)"
        fi
    done
done
    ...
    for ((gpu_idx=0; gpu_idx<GPU_COUNT; gpu_idx++)); do
        ...
        for ((concurrent=0; concurrent<CONCURRENT_PER_GPU; concurrent++)); do
            ...
            if [ $CURRENT_TASK_INDEX -lt $TOTAL_TASKS ]; then
                ...
                (
                    CUDA_VISIBLE_DEVICES=$GPU bash run_exp_imagnet-r.sh --source "$SOURCE" --target "$TARGET" --seed "$SEED" --run "$RUN"
                    echo "====================================="
