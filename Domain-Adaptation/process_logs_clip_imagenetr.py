...
def extract_accuracy_from_log(log_file):
    ...
    try:
        with open(log_file, 'r') as f:
            ...
            match = re.search(r'The best accuracy for target is (\d+\.\d+)', last_line)
            if match:
...
def update_json_with_accuracy(log_dir, output_dir):
    ...
    # 获取所有txt文件
    txt_files = glob.glob(os.path.join(log_dir, "**/*.log"), recursive=True)
    
    for txt_file in txt_files:
        # 从文件路径中提取源域和目标域信息
        # 获取相对于log_dir的路径
        rel_path = os.path.relpath(txt_file, log_dir)
        match = re.search(r'IN-val_INR_seed\d+\.log', rel_path)
        if not match:
            continue
        print("matched ", txt_file)
        source, target = "IN-val", "INR"
...
def main():
    # 设置目录路径
    log_dir = "logs/proposed/clip/imagenet-r"
