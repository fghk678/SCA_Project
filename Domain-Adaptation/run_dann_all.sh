#!/bin/bash
# bash run_dann_all.sh --gpus 0,1
# bash run_dann_all.sh --model "clip ViT-L/14 768" --data_name OfficeHome --data_folder_name OfficeHome
# 默认配置
GPUS=(0 1)
    ...
    for ((gpu_idx=0; gpu_idx<GPU_COUNT; gpu_idx++)); do
        ...
        for ((concurrent=0; concurrent<CONCURRENT_PER_GPU; concurrent++)); do
            ...
            if [ $CURRENT_TASK_INDEX -lt $TOTAL_TASKS ]; then
                ...
                echo "====================================="
                echo "在GPU $GPU 上运行 DANN 实验 $((CURRENT_TASK_INDEX+1))/$TOTAL_TASKS: $SOURCE -> $TARGET \(第 $RUN 次实验, 种子: $SEED\)"
                echo "====================================="
                
                # 后台运行实验，指定GPU
                (
                    bash run_dann_exp.sh --source "$SOURCE" --target "$TARGET" --data_path "$DATADIR" \
                        --model "$MODEL" --data_name "$DATA_NAME" --data_folder_name "$DATA_NAME" \
                        --gpu "$GPU" --epochs "$EPOCHS" --seed "$SEED" --run "$RUN"
                        
                    echo "====================================="
                    echo "GPU $GPU 上的 DANN 实验 $SOURCE -> $TARGET \(第 $RUN 次实验, 种子: $SEED\) 已完成"
                    echo "====================================="
                ) &
...
# 计算所有实验的平均结果
echo "所有 DANN 域适应实验已完成，计算总体平均结果..."
