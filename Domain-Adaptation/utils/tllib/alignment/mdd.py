...
from utils.tllib.modules.grl import WarmStartGradientReverseLayer
...
class GeneralModule(nn.Module):
    def __init__(self, backbone: nn.Module, num_classes: int, bottleneck: nn.Module,
                 ...
                 finetune: Optional[bool] = True, pool_layer: Optional[nn.Module] = None):
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if len(features.shape) == 2:
            # 如果是2D张量，跳过池化层
            f = features.float().detach().squeeze()
        else:
            # 如果是3D或4D张量，使用池化层
            f = self.pool_layer(features)
            
        features = self.bottleneck(f)
...
class ImageClassifier(GeneralModule):
    ...
    def __init__(self, backbone: nn.Module, num_classes: int,
                 bottleneck_dim: Optional[int] = 1024, width: Optional[int] = 1024, in_features: Optional[int] = None, 
        ...
        super(ImageClassifier, self).__init__(backbone, num_classes, bottleneck,
                                              head, adv_head, grl_layer, finetune, pool_layer)
