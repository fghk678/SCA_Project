...
def update_json_with_accuracy(log_dir, output_dir):
    ...
    for txt_file in txt_files:
        ...
        match = re.search(r'OfficeHome_(\w\w)_(\w\w)_seed\d+\.log/train-\d{4}-\d{2}-\d{2}-\d{2}_\d{2}_\d{2}\.txt', rel_path)
        if not match:
            continue
...
def main():
    # 设置目录路径
    log_dir = "logs/mcc/resnet50/OfficeHome"
    output_dir = "output/mcc/resnet50/OfficeHome"
