...
from utils.tllib.alignment.mdd import ClassificationMarginDisparityDiscrepancy \
...
logger.logger.write(f"学习率: lr={args.lr}, momentum={args.momentum}, weight_decay={args.wd}\n")
...
classifier = ImageClassifier(feature, num_classes, bottleneck_dim=args.bottleneck_dim,
                                width=args.bottleneck_dim, in_features=feature_size).to(device)
...
if __name__ == '__main__':
    ...
    if args.phase == 'analysis':
        ...
        logger.logger.write("Saving t-SNE to", tSNE_filename)
        # calculate A-distance, which is a measure for distribution discrepancy
        A_distance = a_distance.calculate(source_feature, target_feature, device)
        logger.logger.write("A-distance =", A_distance)

    if args.phase == 'test':
        acc1 = validate(test_loader, classifier, args, device)
        logger.logger.write("Test accuracy: {:3.1f}\n".format(acc1))
    ...
    logger.logger.write("best_acc1 = {:3.4f}".format(best_acc1))

    # evaluate on test set
    classifier.load_state_dict(torch.load(logger.get_checkpoint_path('best')))
    acc1 = validate(test_loader, classifier, args, device)
    logger.logger.write("test_acc1 = {:3.4f}".format(acc1))
    #  logger.logger.write("Test accuracy: {:3.1f}\n".format(acc1))
