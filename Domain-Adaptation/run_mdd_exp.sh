...
LOGS_DIR="logs/mdd/${MODEL_NAME}/${DATA_NAME}"
...
echo "开始运行 $DATA_NAME 数据集上的 MDD 实验: $SOURCE -> $TARGET \(第 $RUN 次实验, 种子: $SEED\), 模型: $MODEL, GPU: $GPU"
...
# 运行MDD实验
CUDA_VISIBLE_DEVICES=$GPU python mdd.py $DATADIR \
--data_name $DATA_NAME --data_folder_name $DATA_FOLDER_NAME \
--source $SOURCE --target $TARGET --model_type "$MODEL" --i $ITERS_PER_EPOCH \
--epochs $EPOCHS --seed $SEED --log_file "$LOG_FILE" --bottleneck-dim $BOTTLENECK_DIM --trade-off $TRADE_OFF 2>&1 | tee $TEMP_OUTPUT

# 提取最后一行包含 "test_acc1" 的内容
TEST_ACC=$(grep "test_acc1" $TEMP_OUTPUT | tail -n 1)

# 提取实际的准确率数值
ACC_VALUE=$(echo $TEST_ACC | grep -oP "(?<=test_acc1 = )[0-9.]+")
...
echo "$DATA_NAME 数据集上的 MDD 实验 $SOURCE -> $TARGET \(第 $RUN 次实验, 种子: $SEED\) 已完成。" 