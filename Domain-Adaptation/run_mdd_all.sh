#!/bin/bash
# bash run_mdd_all.sh --gpus 0,1
# bash run_mdd_all.sh --model "clip ViT-L/14 768" --data_name OfficeHome --data_folder_name OfficeHome
# 默认配置
GPUS=(0 1)
...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    *)
      ...
      echo "    --i: 每个epoch的迭代次数 (默认: 500)"
      echo "    --epochs: 训练轮数 (默认: 20)"
      echo "    --bottleneck_dim: 瓶颈维度 (默认: 1024)"
      echo "    --runs: 每个源域-目标域组合的实验运行次数 (默认: 5)"
      echo "    --trade_off: 对抗损失权重 (默认: 1.0)"
      exit 1
      ;;
  esac
done
...
OUTPUT_DIR="output/mdd/${MODEL_NAME}/${DATA_NAME}"
mkdir -p $OUTPUT_DIR
    ...
    for ((gpu_idx=0; gpu_idx<GPU_COUNT; gpu_idx++)); do
        ...
        for ((concurrent=0; concurrent<CONCURRENT_PER_GPU; concurrent++)); do
            ...
            if [ $CURRENT_TASK_INDEX -lt $TOTAL_TASKS ]; then
                ...
                echo "====================================="
                echo "在GPU $GPU 上运行 MDD 实验 $((CURRENT_TASK_INDEX+1))/$TOTAL_TASKS: $SOURCE -> $TARGET \(第 $RUN 次实验, 种子: $SEED\)"
                echo "====================================="
                
                # 后台运行实验，指定GPU
                (
                    bash run_mdd_exp.sh --source "$SOURCE" --target "$TARGET" --data_path "$DATADIR" \
                        --model "$MODEL" --data_name "$DATA_NAME" --data_folder_name "$DATA_NAME" \
                        --gpu "$GPU" --i "$ITERS_PER_EPOCH" --epochs "$EPOCHS" --bottleneck_dim "$BOTTLENECK_DIM" \
                        --seed "$SEED" --run "$RUN" --trade_off "$TRADE_OFF"
                        
                    echo "====================================="
                    echo "GPU $GPU 上的 MDD 实验 $SOURCE -> $TARGET \(第 $RUN 次实验, 种子: $SEED\) 已完成"
                    echo "====================================="
                ) &
